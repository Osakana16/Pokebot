...
export namespace pokebot::node {
	...
	public:
		...
		virtual std::optional<engine::HLVector> GetOrigin(const NodeID Node_ID) const = 0;
		virtual size_t GetNumberOfGoals(GoalKind) const = 0;

		virtual void FindPath(PathWalk<std::uint32_t>* const walk_routes, const engine::HLVector& Source, const engine::HLVector& Destination, const game::Team Joined_Team) = 0;
		virtual bool IsNavFileLoaded() const = 0;
		virtual bool IsSameGoal(const NodeID Node_ID, const GoalKind Goal_Kind) const = 0;
		virtual bool IsOnNode(const engine::HLVector& Position, const NodeID Target_Node_ID) const = 0;
		virtual bool HasFlag(const NodeID id, NavmeshFlag flag) const = 0;
		virtual navmesh::NavArea* GetNearest(const engine::HLVector& Destination, const float Beneath_Limit = 120.0f) const = 0;
	};
}